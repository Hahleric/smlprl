...
def get_config():
    ...
    parser.add_argument('--rl_zipf_s', type=float, default=1.2, help='RL 环境中 Zipf 分布参数')
    parser.add_argument('--trad_zipf_s', type=float, default=1.2, help='传统策略中 Zipf 分布参数')
    ...
    parser.add_argument('--use_saved_rl', type=bool, default=False, help='是否使用保存的 RL 模型')

    # RL 环境相关
    parser.add_argument('--num_vehicles', type=int, default=20, help='路口中的车辆数')
    parser.add_argument('--cache_capacity', type=int, default=2, help='RSU 缓存容量，即可缓存的物品数量')
    ...
    parser.add_argument('--testing_step', type=int, default=2048, help='测试时的步数')
    parser.add_argument('--hit_threshold', type=float, default=100, help='命中率阈值, 用于奖励')
    parser.add_argument('--cross_dt', type=float, default=1, help='路口模拟的时间步长')
    parser.add_argument('--spawn_rate', type=float, default=0.2, help='每个时间步生成新车辆的期望数量')
    ...
    parser.add_argument('--hit_reward', type=float, default=20.0, help='缓存命中时的奖励基值')
    parser.add_argument('--miss_penalty', type=float, default=-10.0, help='缓存未命中时的惩罚基值')
