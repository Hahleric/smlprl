...
def get_config():
    ...
    parser.add_argument('--zipf_s', type=float, default=1.5, help='Zipf 分布参数（默认备用，实际使用 rl_zipf_s 或 trad_zipf_s）')
    ...
    parser.add_argument('--sb3_vf_coef', type=float, default=0.5, help='SB3 Critic 损失中的 VF 系数')
    parser.add_argument('--sb3_ent_coef', type=float, default=0.003, help='SB3 Actor 损失中的 Entropy 系数')
    parser.add_argument('--sb3_clip_range', type=float, default=0.1, help='SB3 PPO 损失中的 Clip 范围')
    parser.add_argument('--sb3_lr', type=float, default=0.00005, help='SB3 模型的基础学习率')
    parser.add_argument('--sb3_target_kl', type=float, default=1.0, help='SB3 PPO 损失中的 KL 目标值')
